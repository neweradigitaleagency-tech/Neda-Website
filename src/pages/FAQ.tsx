    ...
    <div className="relative">
      ...
      <section className="pt-40 pb-16 px-6 relative">
        <div className="max-w-3xl mx-auto">
          ...
          >
            <span className="text-sm font-bold uppercase tracking-widest text-[#aaff4e] mb-4 block">{t('faq.label')}</span>
            <h1 className="text-5xl md:text-6xl font-black mb-6 text-default">{t('faq.title')}</h1>
            <p className="text-xl text-muted">{t('faq.desc')}</p>
          </motion.div>
          ...
          >
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-card border border-default focus:border-[#aaff4e] focus:outline-none transition-colors text-default placeholder-muted"
            />
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <AnimatePresence mode="popLayout">
              {filteredQuestions.map((faq, i) => (
                <motion.div
                  ...
                  className="rounded-2xl bg-card border border-default overflow-hidden"
                >
                  <button
                    onClick={() => toggleItem(faq.id)}
                    className="w-full px-6 py-5 text-left flex justify-between items-center gap-4 hover:bg-card transition-colors"
                  >
                    <div className="flex-1">
                      <span className="text-xs text-[#aaff4e] mb-1 block">{t('faq.categories.' + faq.category)}</span>
                      <span className="font-bold text-lg text-default">{faq.q}</span>
                    </div>
                    <ChevronDown className={`w-5 h-5 text-muted shrink-0 transition-transform ${openItems.has(faq.id) ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {openItems.has(faq.id) && (
                      ...
                      >
                        <div className="px-6 pb-5 pt-2 border-t border-default">
          ...
          {filteredQuestions.length === 0 && (
            <div className="text-center py-16">
              <p className="text-muted">{t('faq.noResults')}</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-3xl mx-auto">
          ...
          >
            <h2 className="text-3xl font-black mb-4 text-default">{t('faq.ctaTitle')}</h2>
            <p className="text-muted mb-8">{t('faq.ctaDesc')}</p>
            <a 
